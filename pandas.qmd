...
The `query()` method filters rows according to a specified condition based on values in one or more columns. The query must be passed as a single string. It returns a new data frame containing only the filtered rows, which in our example consists of 842 rows:
...
Notice that we can use columns that we just created to create a new variable like `speed`, where we use the `hours` column which we created in the previous line (but in the same method call). We do have to use a lambda function though, because `df["hours"]` does not exist in the original data frame yet.
