...
Of course, this does not work out of the box and we need to import NumPy before we can use it. By convention, this is done by `import numpy as np`.
...
Each object of type `numpy.ndarray` (short ndarray) has a `dtype` attribute, which indicates the data type of its elements. In our example, all elements are integers (represented by 64 bits):
...
:::{.callout-tip}
In any *n*-dimensional array, the *last* dimension can be interpreted as *columns*, and the *second last* dimension corresponds to *rows*.
:::
...
We can pull out one or more elements of an array using indexing and slicing. This works very similar to lists, at least for one-dimensional arrays. Let's start with a simple one-dimensional array:
...
As a general rule, NumPy carries out operations elementwise. If two arrays have the same shape, this is pretty straightforward:
...
:::{.callout-important}
Whenever possible, avoid loops over arrays. Loops are very slow and inefficient compared to vectorized operations.
:::


### Vector and matrix operations

Note that NumPy does not attach special meaning to 1D or 2D arrays. A 1D array is not interpreted as a vector, and similarly, a 2D array is not interpreted as a matrix. However, it is of course possible to perform vector and matrix operations with special notation. Specifically, the [dot product](https://en.wikipedia.org/wiki/Dot_product) between two vectors can be computed with the `@` operator:
