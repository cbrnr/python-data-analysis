...
### Random numbers
The `numpy.random` module provides functions to generate (pseudo-)random numbers from a variety of distributions. This works as follows: first, we create a generator, which we can then use to draw random numbers from a specific probability distribution. In the following example, we use the default generator provided by `default_rng()`. We also set the random seed of this generator (`42` in this example, but the specific value does not matter), which means that we will get the exact same random numbers every time we run our code. This is important for reproducable results.
...
Finally, the `reshape()` method works just like `resize()`, but it creates a copy and returns the array with the new shape (so it does *not* modify it in place):
...
Remember that the specified axis will disappear from the result! This also works for more than one axis simultaneously:
...
## Useful functions
This sections summarizes some useful functions that we have not discussed so far.
