...
This will create a new directory called `my-project`, containing everything you need to run and reproduce your project. It includes a virtual environment (inside a `.venv` folder), a `pyproject.toml` file, a `uv.lock` file, a `.python-version` file, a `README.md` file, and an example `main.py` script. It will also initialize a Git repository in the project directory and add a `.gitignore` file with some default entries.

Feel free to edit or delete `README.md` and `main.py` – these are just example files to help you get started. However, do not modify or remove the other files, as they are essential for uv to work properly. If you don't want to create all of these files and instead just want a minimal setup, you can use the `--bare` option (note that this also skips Git initialization):
...
This will automatically install the latest version of `numpy` in your project's virtual environment and update the `pyproject.toml` and `uv.lock` files accordingly. From now on, you can use `numpy` in your project. If you want to remove a dependency, you can use `uv remove` followed by the package name. For example, to remove `numpy`, simply run:
...
- `pyproject.toml`: This standard configuration file contains your project's metadata and dependencies. While you can edit it manually, it is recommended to use `uv add` and `uv remove` to manage dependencies.
- `uv.lock`: Generated automatically when you first run a command like `uv run python` in your environment, this file captures an exact snapshot of your project's dependencies (with exact version numbers). It ensures your project remains reproducible, even if upstream packages change in the future. You should *never* edit this file manually!
