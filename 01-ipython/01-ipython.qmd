...
Running `uv run python` (without any further arguments) from a [command line shell](https://en.wikipedia.org/wiki/Shell_(computing)) (such as *Terminal* on macOS or *Windows Terminal* on Windows) invokes the *interactive* Python interpreter. This interpreter mode is especially useful for interactively playing around with data using a workflow called the read–eval–print loop ([REPL](https://en.wikipedia.org/wiki/Read%E2%80%93eval%E2%80%93print_loop)), in which Python *reads* a command (which the user just entered), *evaluates* it, *prints* its result, and repeats these steps for every subsequent command in a *loop*.
...
If you are using uv to manage your Python projects, you can simply start IPython in your project directory by running the following command in the terminal:
...
In addition to the last result, we can also retrieve the two previous ones with double and triple underscores, respectively. Alternatively, we can access a specific output by name via `_1`, `_2`, etc. (the numbers correspond to the output number). This also works for inputs using `_i1`, `_i2`, and so on.
...
- `%pycat <file.txt>` displays the contents of the file `<file.txt>` with syntax highlighting.
...
:::{.callout-important}
Magic commands are *not* valid Python, so use them only in the interactive interpreter – do *not* use them in Python scripts!
:::
...
Therefore, always check if there are suitable magic commands available before using shell commands. For example, listing the contents of the working directory can be achieved with `%ls`, and showing the contents of a file can be done with `%pycat`. These magic commands are cross-platform and work regardless of the underlying shell or operating system.
:::
...
::: {.callout-note}
Quarto and Jupyter support languages other than Python, for example R and Julia.
:::
