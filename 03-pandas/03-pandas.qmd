...
[Pandas](https://pandas.pydata.org/)[^2] offers a flexible data type for tabular data which builds upon NumPy arrays, but can accommodate heterogeneous column types: the so-called *data frame*.

[^2]: Note that pandas is spelled with all lower-case letters. We will capitalize the first letter only at the beginning of a sentence.
...
::: callout-note
[Polars](https://pola.rs/) is a newer package that is similar to pandas, but it often provides better performance due to its Rust-based, multi-threaded lazy execution engine. At present, pandas remains more widely supported and is still the standard in most scientific Python workflows, but this may change in the future. However, this is a topic for another course.
:::

## Series

A pandas series (`pd.Series`) is a sequence-like homogeneous object, similar to a one-dimensional NumPy array. Series are often based on NumPy arrays under the hood, so they share important properties. For example, just like a NumPy array, a series has a specific data type (which means that all elements have the same type).
...
The output shows two columns: the values are in the right column, whereas the left column shows the index (which by default starts at zero). The bottom row displays the data type (`int64` in this case).
...
Pandas data types are based on [NumPy array scalars](https://numpy.org/doc/stable/reference/arrays.scalars.html). The most common [sized numerical types](https://numpy.org/doc/stable/reference/arrays.scalars.html#sized-aliases) are:
...
Since pandas 3.0, strings are stored as `str` by default:

```{python}
pd.Series(["a", "b", "c"])
```

In older versions of pandas, strings were stored as `object` – a NumPy data type that acts as a generic container for arbitrary Python objects – which made string columns ambiguous and less efficient.
...
- `boolean` for boolean data with support for missing values (note that this is different from the basic `bool` dtype)
...
In general, it is recommended to use extension data types for integers and booleans because they offer better support for missing values.

::: callout-note
Pandas also has an older `string` extension dtype (specified as `dtype="string"`). The key difference is how missing values are represented: `str` uses `np.nan` (consistent with other default data types), whereas `string` uses `pd.NA`. Unless you specifically need `pd.NA` semantics, use the default `str`.
:::
...
In this example, we see that `df` consists of four rows and three columns. For each column, the label, number of non-missing (non-null) values, and data type are listed. Column A contains `float64` values (double precision floating point numbers), column B contains strings (`str`), and column C contains `int64` integers.
...
We can change this behavior by setting the following two options to appropriate values (`None` means unlimited, whereas `0` means the defaults):
...
We can round the values in this table for a more compact display:
...
Almost any real-word data contains missing values. Pandas encodes missing numeric values as `NaN` ("Not a Number"), a sentinel value defined in the [IEEE 754 floating point standard](https://en.wikipedia.org/wiki/IEEE_754). Because `NaN` is a floating point concept, integer series are automatically promoted to `float64` when they contain missing values. Datetime types have a special `NaT` ("Not a Time") value to represent missing dates.

Pandas also has a native `NA` ("Not Available") value (`pd.NA`) that is not tied to any specific storage type. Extension data types such as `Int64` and `boolean` use `pd.NA` for missing values, displayed as `<NA>`.
...
Notice that the first column is promoted to `float64` because it contains a missing value (otherwise it would be `int64`). Both numeric columns represent missing values as `NaN`. The last column is inferred as `object` because it also contains `None` (interpreted as a missing value).

These types may not be what we want. For integers and booleans in particular, it is a good idea to use extension data types, which preserve the intended type even in the presence of missing values:
...
This works for all kinds of missing value representations, including `NaN`, `NaT`, `None`, and `NA`.
:::
...
Columns `name` and `gender` contain strings (so their data type is `str`), columns `id` and `height` contain integers (`int64`), and column `bp` holds floating point numbers (`float64`). Although column `weight` originally contains only integers, pandas promotes it to `float64` due to the presence of a missing value.

If we are not happy with one of the automatically inferred data types, we can always change individual columns after importing with the `astype()` method. For example, the `gender` column could be represented with a categorical data type. We could also use the `Int64` data type for the `weight` column, which supports missing values.
...
```{python}
df = pd.read_csv("data.csv", dtype={"gender": "category", "weight": "Int64"})
df.dtypes
```

Let's now take a look at another text file [`data.txt`](data.txt) with the following contents:
...
Other useful functions for reading text data include `pd.read_clipboard()`, `pd.read_fwf()`, `pd.read_json()`, and `pd.read_xml()`. Consult the [pandas documentation](https://pandas.pydata.org/docs/user_guide/io.html) to learn more.
...
Let's import data from an Excel file named [`data.xlsx`](data.xlsx) (located in the working directory), which contains the same table we saw in the previous section:

```{python}
pd.read_excel("data.xlsx")
```

We can use many of the arguments we already know from `pd.read_csv()` (such as `header` and `dtype`). In addition, Excel files can contain multiple sheets. To specify which sheet you want to import, use the `sheet_name` argument. For more information on all available parameters, consult the [`pd.read_excel()` reference](https://pandas.pydata.org/docs/reference/api/pandas.read_excel.html).
...
Data wrangling can be broken down into the following five basic tasks (as defined in [R for Data Science](https://r4ds.hadley.nz/data-transform#summary)):
...
We will illustrate each operation using the `flights` data frame from the [nycflights13](https://github.com/tidyverse/nycflights13) data set. This data frame is available as a zipped CSV file [`flights.zip`](flights.zip) in the working directory, and we can import it as follows:
...
Notice that in order to get better support for missing values, we specify the data types for integer columns manually as the extension type `Int64`. We also drop the `year`, `hour`, `minute`, and `time_hour` columns, because we won't need them in our analyses (and they contain only redundant information anyway).
...
Regular expressions are a powerful way to describe text patterns. We do not have time to go into more details, but it pays off to learn at least the basic concepts (for example with [this tutorial](https://regexone.com/)).
...
Pandas makes it pretty straightforward to convert any given table to a tidy format (or vice versa) using the `pivot()` method. Let's start with data from @tbl-messy-1 and try to make it tidy:
...
Let's apply some of the things we learned in this chapter to a real-world data set: historic COVID-19 cases in Austria. The data is not available on any of the official government websites anymore, but we can use a snapshot from June 28 2023 (renamed to [`covid19.csv`](covid19.csv)) and put it in our working directory.
